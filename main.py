...
class MyBot(BaseBot):
    def __init__(self):
        ...
        self.active_emote_is_random = False
    ...
    def is_bot(self, user_id=None, username=None):
        if user_id and self.bot_id:
            return user_id == self.bot_id
        if username and self.bot_username:
            return username.lower() == self.bot_username.lower()
        return False

    def set_active_emote(self, emote_name, is_random=False):
        self.active_emote = emote_name
        self.active_emote_is_random = is_random

    def get_active_emote(self):
        return self.active_emote, self.active_emote_is_random

    def clear_active_emote(self):
        self.active_emote = None
        self.active_emote_is_random = False
        ...
        try:
            ...
            for room_user, position in room_users.content:
                if room_user.id == self.bot_id:
                    ...
                    break
            print(f"Bot bilgileri - ID: {self.bot_id}, Username: {self.bot_username}")
    ...
    async def on_emote(self, user: User, emote_id: str, receiver: User) -> None:
        ...
        if receiver and self.is_bot(user_id=receiver.id):
            print(f"Emote alındı: {user.username} -> Bot")
        elif user and self.is_bot(user_id=user.id):
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        # Individual moderation commands - show usage if just command without parameters
            ...
            if not self.is_bot(user_id=user.id):
    ...
    async def handle_give_command(self, user: User, message: str) -> None:
        ...
        if self.is_bot(user_id=target_user_in_room.id, username=target_user):
        ...
        if self.role_manager.add_user_to_role(target_user, role):
            ...
            if not self.is_bot(user_id=target_user_in_room.id):
    ...
    async def send_reaction_to_all(self, reaction_type: str, amount: int, sender_id: str) -> int:
        ...
        try:
            ...
            for room_user, position in room_users.content:
                ...
                if self.is_bot(user_id=room_user.id):
                    continue
    ...
    async def send_tip_to_all(self, sender: User, amount: int) -> None:
        ...
        try:
            ...
            for room_user, position in room_users.content:
                ...
                if (self.is_bot(user_id=room_user.id) or 
    ...
    async def send_tip_to_user(self, sender: User, target_username: str, amount: int) -> None:
        ...
        if self.is_bot(user_id=target_user.id):
        ...
        try:
            ...
            if response == "success":
                ...
                if not self.is_bot(user_id=target_user.id):
